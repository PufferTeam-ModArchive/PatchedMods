...
public class BlockProxy {
    ...
    public BlockProxy getNeighbor(int side) {
        return new BlockProxy(
            world,
            coords.copy()
                .add(FacingUtil.getOffsetForSide(side)));
    }
    ...
    public int getPowerInputFrom(int side) {
        ...
        if (neighbor != null) {
            int weak = neighbor.isProvidingWeakPower(
                world,
                neighProxy.getCoords().x,
                neighProxy.getCoords().y,
                neighProxy.getCoords().z,
                side);
            int strong = neighbor.isProvidingStrongPower(
                world,
                neighProxy.getCoords().x,
                neighProxy.getCoords().y,
                neighProxy.getCoords().z,
    ...
    private void move(int dir, float speed, Vector3 target) {
        ...
        if (this.world.isRemote) {
            ModClass.proxy.getFakeWorld(world)
                .clearFakeDataAt(coords);
            ModClass.proxy.getFakeWorld(world)
                .clearFakeDataAt(target);

            FakeBlockData data = new FakeBlockData(
                target.x,
                target.y,
                target.z,
                BlockItemUtil.getBlockID(storedBlock),
                storedMetadata);
            ModClass.proxy.getFakeWorld(world)
                .addFakeBlockData(data);

            if (storedTl != null) {
                ModClass.proxy.getFakeWorld(world)
                    .addFakeTl(storedTl, target);
            }
        }
    ...
    public void move(int dir, float speed) {
        Vector3 target = coords.copy()
            .add(FacingUtil.getOffsetForSide(dir));
    ...
    public TileEntity getTileEntityForRotate(Vector3 rotatePoint, int rotateDir, float speed) {
        ...
        return tile;
        // return BlockMechRotating.getTileEntity(storedBlockID, storedMetadata, storedTl, storedNBT, 1, true,
        // rotatePoint, rotateDir, speed);
    }
    ...
    public void rotateWithTileEntity(Vector3 rotatePoint, int rotateDir, TileEntity tile) {
        ...
        if (this.world.isRemote) {
            ...
            FakeBlockData data = new FakeBlockData(
                ...
                target.y,
                target.z,
                BlockItemUtil.getBlockID(motion.getElement()),
                storedMetadata);
            ModClass.proxy.getFakeWorld(world)
                .addFakeBlockData(data);
    ...
    public boolean equals(Object obj) {
        ...
        if (coords == null) {
            if (other.coords != null) return false;
        } else if (!coords.equals(other.coords)) return false;
        return true;
    }
