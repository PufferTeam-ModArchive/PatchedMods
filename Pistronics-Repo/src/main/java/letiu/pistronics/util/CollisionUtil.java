...
public class CollisionUtil {
    ...
    public static MovingObjectPosition multiCollisionRayTrace(World world, int x, int y, int z, Vec3 playerPos,
        Vec3 targetPos, ArrayList<AxisAlignedBB> boxes) {
        ...
        for (int i = 0; i < boxes.size(); i++) {
            ...
            AxisAlignedBB box = boxes.get(i)
                .getOffsetBoundingBox((double) (-x), (double) (-y), (double) (-z));
            ...
            if (isVecInsideYZBounds(vec31, box)) {
                if (currentVec == null
                    || (playerPos.squareDistanceTo(vec31) < playerPos.squareDistanceTo(currentVec))) {
                    currentVec = vec31;
                    currentSide = 4;
                    subHit = i;
                }
            ...
            if (isVecInsideYZBounds(vec32, box)) {
                if (currentVec == null
                    || (playerPos.squareDistanceTo(vec32) < playerPos.squareDistanceTo(currentVec))) {
                    currentVec = vec32;
                    currentSide = 5;
                    subHit = i;
                }
            ...
            if (isVecInsideXZBounds(vec33, box)) {
                if (currentVec == null
                    || (playerPos.squareDistanceTo(vec33) < playerPos.squareDistanceTo(currentVec))) {
                    currentVec = vec33;
                    currentSide = 0;
                    subHit = i;
                }
            ...
            if (isVecInsideXZBounds(vec34, box)) {
                if (currentVec == null
                    || (playerPos.squareDistanceTo(vec34) < playerPos.squareDistanceTo(currentVec))) {
                    currentVec = vec34;
                    currentSide = 1;
                    subHit = i;
                }
            ...
            if (isVecInsideXYBounds(vec35, box)) {
                if (currentVec == null
                    || (playerPos.squareDistanceTo(vec35) < playerPos.squareDistanceTo(currentVec))) {
                    currentVec = vec35;
                    currentSide = 2;
                    subHit = i;
                }
            ...
            if (isVecInsideXYBounds(vec36, box)) {
                if (currentVec == null
                    || (playerPos.squareDistanceTo(vec36) < playerPos.squareDistanceTo(currentVec))) {
                    currentVec = vec36;
                    currentSide = 3;
                    subHit = i;
                }
        ...
        MovingObjectPosition result = new MovingObjectPosition(
            ...
            z,
            currentSide,
            currentVec.addVector((double) x, (double) y, (double) z));
        result.subHit = subHit;
        return result;
    }

    public static MovingObjectPosition multiCollisionRayTraceWithSubHitMap(World world, int x, int y, int z,
        Vec3 playerPos, Vec3 targetPos, ArrayList<AxisAlignedBB> boxes, ArrayList<Byte> subHitMap) {
    ...
    public static boolean isVecInsideYZBounds(Vec3 vec, AxisAlignedBB box) {
        return vec != null && vec.yCoord >= box.minY
            && vec.yCoord <= box.maxY
            && vec.zCoord >= box.minZ
            && vec.zCoord <= box.maxZ;
    }
    ...
    public static boolean isVecInsideXZBounds(Vec3 vec, AxisAlignedBB box) {
        return vec != null && vec.xCoord >= box.minX
            && vec.xCoord <= box.maxX
            && vec.zCoord >= box.minZ
            && vec.zCoord <= box.maxZ;
    }
