...
public class BExtension extends PBlock implements IPistonElement, ISpecialRotator {
        ...
        if (meta == side) {
            ...
            if (tileExt.redstone) {
                if (tileExt.isActive()) {
                    if (tileExt.camou) {
                        if (tileExt.redio) {
                            return tileExt.super_sticky ? Textures.REDIO_CAMOU_SUPER_EXT_ON
                                : Textures.REDIO_CAMOU_EXT_ON;
                        } else return tileExt.super_sticky ? Textures.RS_EXT_CAMOU_ON_S_S
                            : tileExt.sticky ? Textures.RS_EXT_CAMOU_ON_S : Textures.RS_EXT_CAMOU_ON;
                    } else {
                        if (tileExt.redio) {
                            return tileExt.super_sticky ? Textures.REDIO_SUPER_EXT_ON : Textures.REDIO_EXT_ON;
                        } else return tileExt.super_sticky ? Textures.RS_EXT_ON_S_S
                            : tileExt.sticky ? Textures.RS_EXT_ON_S : Textures.RS_EXT_ON;
                    }
                } else {
                    if (tileExt.camou) {
                        if (tileExt.redio) {
                            return tileExt.super_sticky ? Textures.REDIO_CAMOU_SUPER_EXT_OFF
                                : Textures.REDIO_CAMOU_EXT_OFF;
                        } else return tileExt.super_sticky ? Textures.RS_EXT_CAMOU_OFF_S_S
                            : tileExt.sticky ? Textures.RS_EXT_CAMOU_OFF_S : Textures.RS_EXT_CAMOU_OFF;
                    } else {
                        if (tileExt.redio) {
                            return tileExt.super_sticky ? Textures.REDIO_SUPER_EXT_OFF : Textures.REDIO_EXT_OFF;
                        } else return tileExt.super_sticky ? Textures.RS_EXT_OFF_S_S
                            : tileExt.sticky ? Textures.RS_EXT_OFF_S : Textures.RS_EXT_OFF;
                    }
                }
            } else {
                if (tileExt.camou) {
                    return tileExt.super_sticky ? Textures.CAMOU_EXT_SUPER_STICKY
                        : tileExt.sticky ? Textures.CAMOU_EXT_STICKY : Textures.CAMOU_EXT;
                } else return tileExt.super_sticky ? Textures.EXTENSION_SUPER_STICKY
                    : tileExt.sticky ? Textures.EXTENSION_STICKY : Textures.EXTENSION_NORMAL;
            }
        } else if (meta == (side ^ 1)) {
            if (tileExt.redstone) {
                if (tileExt.isActive()) {
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int side) {
        int facing = WorldUtil.getBlockFacing(
            world,
    ...
    public ArrayList<AxisAlignedBB> getBoxes(IBlockAccess world, int x, int y, int z, int meta) {
        ...
        switch (meta & 7) {
            case 0:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0D, 0.0D, 0.0D, 1.0D, 0.25D, 1.0D));
                boxes.add(AxisAlignedBB.getBoundingBox(0.375D, 0.25D, 0.375D, 0.625D, 1.0D, 0.625D));
                break;
            case 1:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0D, 0.75D, 0.0D, 1.0D, 1.0D, 1.0D));
                boxes.add(AxisAlignedBB.getBoundingBox(0.375D, 0.0D, 0.375D, 0.625D, 0.75D, 0.625D));
                break;
            case 2:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 0.25D));
                boxes.add(AxisAlignedBB.getBoundingBox(0.375D, 0.375D, 0.25D, 0.625D, 0.625D, 1.0D));
                break;
            case 3:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0D, 0.0D, 0.75D, 1.0D, 1.0D, 1.0D));
                boxes.add(AxisAlignedBB.getBoundingBox(0.375D, 0.375D, 0.0D, 0.625D, 0.625D, 0.75D));
                break;
            case 4:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0D, 0.0D, 0.0D, 0.25D, 1.0D, 1.0D));
                boxes.add(AxisAlignedBB.getBoundingBox(0.25D, 0.375D, 0.375D, 1.0D, 0.625D, 0.625D));
                break;
            case 5:
                boxes.add(AxisAlignedBB.getBoundingBox(0.75F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F));
                boxes.add(AxisAlignedBB.getBoundingBox(0.0D, 0.375D, 0.375D, 0.75D, 0.625D, 0.625D));
                break;
            default:
                return null;
        }
    ...
    public MovingObjectPosition collisionRayTrace(World world, int x, int y, int z, Vec3 playerPos, Vec3 targetPos) {
        ...
        return CollisionUtil
            .multiCollisionRayTraceWithSubHitMap(world, x, y, z, playerPos, targetPos, boxes, subHitMap);
    }
    ...
    protected boolean handleGUILCrafting(World world, int x, int y, int z, EntityPlayer player, int side,
        TileExtension tile, int facing, ItemStack stack) {

        if (tile == null) return false;

        if (facing == side) {
            if (!tile.super_sticky && !tile.sticky
                && tile.redstone
                && (CompareUtil.compareIDs(stack, ItemData.redioSuperGlue.item))) {
                tile.sticky = true;
            ...
            if (!tile.super_sticky && !tile.sticky
                && tile.redstone
                && (CompareUtil.compareIDs(stack, ItemData.redioGlue.item))) {
                tile.sticky = true;
                tile.redio = true;
                if (!player.capabilities.isCreativeMode) stack.stackSize--;
                return true;
            ...
            if (!tile.super_sticky && !tile.sticky
                && (CompareUtil.compareIDs(stack, ItemReference.SLIME)
                    || CompareUtil.compareIDs(stack, ItemData.glue.item))) {
                tile.sticky = true;
                if (!player.capabilities.isCreativeMode) stack.stackSize--;
                return true;
            }
                ...
                if (tile.redstone) {
                    ...
                    if (!world.isRemote && !player.capabilities.isCreativeMode) {
                        if (this instanceof BExtensionPart) {
                            ...
                            WorldUtil
                                .spawnItemStack(world, x, y, z, BlockItemUtil.getStack(ItemReference.REDSTONE_ITEM));
                        }
                    }
                    return true;
                }
                    ...
                    if (neighbor.isSolid()) {
                        if (neighbor.isPistonElement()) {
                            ...
                            if (neighbor.getPBlock() instanceof BRodFolder) {
                                if (eFacing == antiDir
                                    && system.isInsertable(neighbor, neighbor.getNeighbor(antiDir))) {
                                    neighbor.getConnectedForSystem(system, false);
                                ...
                                if ((eFacing == antiDir || eFacing == dir)
                                    && system.isInsertable(neighbor, neighbor.getNeighbor(antiDir))) {
                                    neighbor.getConnectedForSystem(system, false);
                                } else neighbor.getConnectedForSystem(system, true);
                            } else neighbor.getConnectedForSystem(system, true);
    ...
    public boolean isTransmitter(BlockProxy proxy) {
        ...
        if (tile != null && tile instanceof TileExtension) return ((TileExtension) tile).isConductive();
        else return false;
    }
}
