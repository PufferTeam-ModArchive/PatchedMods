...
public class BStatue extends PBlock implements ISpecialRotator {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase player, ItemStack stack) {
        ...
        if (pTile != null && pTile instanceof TileStatue) {
            ...
            if (stack.stackTagCompound != null) tile.readFromNBT(stack.stackTagCompound);
            ...
            if (stack != null) {
                ...
                if (CompareUtil.compareIDs(stack, ItemData.camoupaste.item)) {
                    ...
                    if (!world.isRemote) PacketHandler
                        .sendToAllInDimension(new StatueDataPacket(tile, player.dimension), player.dimension);
                    return true;
                }
                    ...
                    if (CompareUtil.compareIDs(stack, ItemReference.MILK)) {
                        ...
                        if (!player.capabilities.isCreativeMode) {
                            player.inventory.setInventorySlotContents(
                                ...
                                BlockItemUtil.getStack(ItemReference.BUCKET));
                        }
                        if (!world.isRemote) PacketHandler
                            .sendToAllInDimension(new StatueDataPacket(tile, player.dimension), player.dimension);
                        return true;
