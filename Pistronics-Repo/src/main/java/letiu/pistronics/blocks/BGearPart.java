...
public class BGearPart extends PBlock implements IPistonElement, ISpecialRotator {
    ...
    public ArrayList<AxisAlignedBB> getBoxes(IBlockAccess world, int x, int y, int z, int meta) {
        ...
        int gearSize = 0; // Math.max(Math.max(Math.abs(gearPos.x - x), Math.abs(gearPos.y - y)), Math.abs(gearPos.z -
                          // z));
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float xHit,
        ...
        return BlockData.gear
            .onBlockActivated(world, gearCoords.x, gearCoords.y, gearCoords.z, player, side, xHit, yHit, zHit);
    }
    ...
    public void onBlockBreak(World world, int x, int y, int z, Block block2, int meta) {
        ...
        BlockData.gear.onBlockBreak(
            world,
            gearPos.x,
            gearPos.y,
            gearPos.z,
            BlockData.gear.block,
