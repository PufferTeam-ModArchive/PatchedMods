...
public class BRod extends PBlock implements IPistonElement, ISpecialRotator {
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int side) {
        ...
        return true;
        /*
         * int facing = WorldUtil.getBlockFacing(world, x - Facing.offsetsXForSide[side], y -
         * Facing.offsetsYForSide[side], z - Facing.offsetsZForSide[side]);
    ...
    public ArrayList<AxisAlignedBB> getBoxes(IBlockAccess world, int x, int y, int z, int meta) {
        ...
        switch (meta & 7) {
            case 1:
            case 0:
                box = AxisAlignedBB.getBoundingBox(0.375D, 0.0D, 0.375D, 0.625D, 1.0D, 0.625D);
                break;
            case 2:
            case 3:
                box = AxisAlignedBB.getBoundingBox(0.375D, 0.375D, 0.0D, 0.625D, 0.625D, 1.0D);
                break;
            case 4:
            case 5:
                box = AxisAlignedBB.getBoundingBox(0.0D, 0.375D, 0.375D, 1.0D, 0.625D, 0.625D);
                break;
        }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float xHit,
                ...
                if (ItemReference.isRedstone(stack)) {
                    if (!world.isRemote) {
                        ...
                        PacketHandler.sendToAllInDimension(
                            new RodExDataPacket(tile, player.dimension, inMachine),
                            player.dimension);
                    }
                    return true;
                }
        ...
        if (tile != null && tile instanceof TileRod && ItemReference.isOre(stack, "slabWood")) {
            ...
            if (!player.capabilities.isCreativeMode) stack.stackSize--;
            if (inMachine && !world.isRemote)
                PacketHandler.sendToAllInDimension(new RodExDataPacket(tile, player.dimension, true), player.dimension);
            return true;
        }
        ...
        if (CompareUtil.compareIDs(stack, ItemData.spade.item)) {
            if (((TileRod) tile).redstone) {
                ...
                if (!world.isRemote) {
                    ...
                    PacketHandler
                        .sendToAllInDimension(new RodExDataPacket(tile, player.dimension, inMachine), player.dimension);
                }
        ...
        if (!inMachine && CompareUtil.compareIDs(stack, ItemData.saw.item)) {
            ...
            if (!world.isRemote) {
                if (!player.capabilities.isCreativeMode) {
                    ...
                    if (tile != null && tile instanceof TileRod)
                        drop.stackTagCompound = ((TileRod) tile).getNBTForItem();
            ...
            if (side != facing && side != (facing ^ 1)) {
                ...
                if (pItem != null && pItem instanceof IPart && ((IPart) pItem).canPartBeAdded(world, x, y, z, side)) {
                    ...
                    if (partblockTile != null) {
                        ...
                        ((IPart) part).onPartPlaced(
                            partblockTile,
                            partTile,
                            side,
                            MiscUtil.getPartFromClick(xHit, yHit, zHit),
                            player);
    ...
    public boolean isTransmitter(BlockProxy proxy) {
        ...
        if (tile != null && tile instanceof TileRod) return ((TileRod) tile).isConductive();
        else return false;
    }
}
