...
public class BGear extends PBlock implements IPistonElement, ISpecialRotator {
    ...
    public ArrayList<AxisAlignedBB> getBoxes(IBlockAccess world, int x, int y, int z, int meta) {
        ...
        switch (meta & 7) {
            case 1:
            case 0:
                box = AxisAlignedBB.getBoundingBox(-gearSize, 0.0D, -gearSize, gearSize + 1, 1.0D, gearSize + 1);
                break;
            case 2:
            case 3:
                box = AxisAlignedBB.getBoundingBox(-gearSize, -gearSize, 0.0D, gearSize + 1, gearSize + 1, 1.0D);
                break;
            case 4:
            case 5:
                box = AxisAlignedBB.getBoundingBox(0.0D, -gearSize, -gearSize, 1.0D, gearSize + 1, gearSize + 1);
                break;
        }
    ...
    public static void createGearBlock(World world, int x, int y, int z, int facing, int size, int plankMeta,
        boolean crafted) {
            ...
            if (stack == null && gearTile.hasRod) {
                if (facing == side) {
                    if (!world.isRemote) {
                        ...
                        PacketHandler
                            .sendToAllInDimension(new GearDataPacket(gearTile, player.dimension), player.dimension);
                    }
                    return true;
                        ...
                        if (gear.size < size || checkGearRingSpace(world, x, y, z, facing, gear.size)) {
                            ...
                            if (!world.isRemote) {
                                ...
                                PacketHandler.sendToAllInDimension(
                                    new GearDataPacket(gearTile, player.dimension),
                                    player.dimension);
                            }
                            return true;
                        }
    ...
    public static boolean checkGearRingSpace(World world, int x, int y, int z, int facing, int size) {
        ...
        switch (facing) {
            case 1:
            case 0:
                return MiscUtil.checkSurroundings(
                    world,
                    new Vector3(x, y, z),
