...
public class BRodPart extends BRod implements IPart {
    ...
    public ArrayList<AxisAlignedBB> getBoxes(IBlockAccess world, int x, int y, int z, int meta) {
        ...
        switch (meta & 7) {
            case 1:
                box = AxisAlignedBB.getBoundingBox(0.375D, 0.625D, 0.375D, 0.625D, 1D, 0.625D);
                break;
            case 0:
                box = AxisAlignedBB.getBoundingBox(0.375D, 0.0D, 0.375D, 0.625D, 0.375D, 0.625D);
                break;
            case 5:
                box = AxisAlignedBB.getBoundingBox(0.625D, 0.375D, 0.375D, 1D, 0.625, 0.625D);
                break;
            case 4:
                box = AxisAlignedBB.getBoundingBox(0D, 0.375D, 0.375D, 0.375D, 0.625D, 0.625D);
                break;
            case 3:
                box = AxisAlignedBB.getBoundingBox(0.375D, 0.375D, 0.625D, 0.625D, 0.625D, 1D);
                break;
            case 2:
                box = AxisAlignedBB.getBoundingBox(0.375D, 0.375D, 0D, 0.625D, 0.625D, 0.375D);
                break;
        }
                    ...
                    if (((TileRod) partTile).redstone) {
                        ...
                        if (!world.isRemote && !player.capabilities.isCreativeMode) {
                            if (this instanceof BRodPart) {
                                WorldUtil
                                    .spawnItemStack(world, x, y, z, BlockItemUtil.getStack(ItemData.pileOfRedstone));
                            } else {
                                WorldUtil.spawnItemStack(
                                    world,
                                    x,
                                    y,
                                    z,
    ...
    public void getConnectedForSystem(PistonSystem system, BlockProxy proxy, TilePartblock tile, int part) {
        ...
        if (system.getSystemType() == SystemType.REDSTONE && !((ITransmitter) tile.getTile(part)).isConductive())
            return;
