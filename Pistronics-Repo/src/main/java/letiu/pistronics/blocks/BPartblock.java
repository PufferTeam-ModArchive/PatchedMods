...
public class BPartblock extends PBlock implements IPistonElement, ISpecialRotator {
    ...
    public String getTextureIndex(PTile tile, int meta, int side) {
        ...
        if (partMeta != -1) {
            ...
            return tilePB.getPart(partMeta)
                .getTextureIndex(partTile, partMeta, side);
        }
    ...
    public ArrayList<AxisAlignedBB> getBoxesWithSubHitMap(World world, int x, int y, int z, int meta,
        List<Byte> subHitMap) {
        ArrayList<AxisAlignedBB> boxes = new ArrayList<AxisAlignedBB>();
            ...
            if (!tile.hasPart(side) && item != null && (item instanceof IBaseItem)) {
                ...
                if (pItem != null && pItem instanceof IPart && ((IPart) pItem).canPartBeAdded(world, x, y, z, side)) {
                    if (!world.isRemote) {
                        ...
                        ((IPart) part)
                            .onPartPlaced(tile, partTile, side, MiscUtil.getPartFromClick(xHit, yHit, zHit), player);

                        if (!player.capabilities.isCreativeMode) stack.stackSize--;
                        PacketHandler.sendToAllInDimension(
                            new RodExDataPacket(partTile, side, player.dimension),
                            player.dimension);
                    }
                    return true;
                    ...
                    if (!tile.hasPart(side ^ 1)) {
                        if (!world.isRemote) {
                            ...
                            PacketHandler.sendToAllInDimension(
                                new RodExDataPacket(partTile, side ^ 1, player.dimension),
                                player.dimension);
            ...
            if (!hasCamouSail) {
                ...
                if (partTile == null || !(partTile instanceof TileExtension) || !((TileExtension) partTile).camou) {
                    if (stack.getItem() instanceof ItemBlock
                        && !CompareUtil.compareIDs(stack, ItemReference.WOODEN_SLAB)) return false;
                }
            }
    ...
    public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z) {
        ...
        if (tile != null) {
            if (target.subHit >= 0 && target.subHit < 6) {
                return tile.getPart(target.subHit)
                    .getDroppedStack(tile.getTile(target.subHit), 0);
            }
        }
