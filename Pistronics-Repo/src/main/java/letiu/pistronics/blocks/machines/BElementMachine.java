...
public abstract class BElementMachine extends BBaseMachine implements IPistonElement, ISpecialRotator {
    ...
    public ArrayList<AxisAlignedBB> getBoxes(IBlockAccess world, int x, int y, int z, int meta) {
        ...
        switch (meta & 7) {
            case 0:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0F, 0.25F, 0.0F, 1.0F, 1.0F, 1.0F));
                break;
            case 1:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0F, 0.0F, 0.0F, 1.0F, 0.75F, 1.0F));
                break;
            case 2:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0F, 0.0F, 0.25F, 1.0F, 1.0F, 1.0F));
                break;
            case 3:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 0.75F));
                break;
            case 4:
                boxes.add(AxisAlignedBB.getBoundingBox(0.25F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F));
                break;
            case 5:
                boxes.add(AxisAlignedBB.getBoundingBox(0.0F, 0.0F, 0.0F, 0.75F, 1.0F, 1.0F));
                break;
            default:
                return null;
        }
                        ...
                        if (CompareUtil.compareIDs(stack, ItemData.camoupaste.item)) {
                            if (!world.isRemote) {
                                ...
                                PacketHandler.sendToAllInDimension(
                                    new RodExDataPacket(tileMech, player.dimension),
                                    player.dimension);
                            }
                            return true;
                        }
                        ...
                        if (block != null && block.getRenderType() == 0) {
                            ...
                            if (!world.isRemote) {
                                if (tileMech.camouID != -1) {
                                    ...
                                    if (!player.capabilities.isCreativeMode) WorldUtil
                                        .spawnItemStack(world, x, y, z, BlockItemUtil.getStack(ItemData.camoupaste));
                                }
                                PacketHandler.sendToAllInDimension(
                                    new RodExDataPacket(tileMech, player.dimension),
                                    player.dimension);
            ...
            if (stack == null && player.isSneaking() && tileEH.hasElement()) {
                ...
                if (!world.isRemote) {
                    if (!player.capabilities.isCreativeMode) {
                        ...
                        tileEH.getPElementTile()
                            .writeToNBT(drop.stackTagCompound);

                        WorldUtil.spawnItemStack(world, x, y, z, drop);
                    }
                    tileEH.clear();
                    PacketHandler.sendToAllInDimension(
                        new RodExDataPacket(tileEH, player.dimension, true, true),
                        player.dimension);
                }
            ...
            if (FacingUtil.getRelevantAxis(facing, xHit, yHit, zHit) > 0.75F) {
                ...
                if (element instanceof BRod && ItemReference.isOre(stack, "slabWood")) {
                    ...
                    if (!world.isRemote) PacketHandler
                        .sendToAllInDimension(new RodExDataPacket(tileEH, player.dimension, true), player.dimension);
                    return true;
                }
    ...
    public void onBlockHarvested(World world, int x, int y, int z, int fortune, EntityPlayer player) {
        ...
        if (!player.capabilities.isCreativeMode || ItemReference.isHarvestTool(stack)) {
            ...
            if (tile.hasElement()) {
                ...
                drop.stackTagCompound = tile.getPElementTile()
                    .getNBTForItem();
                WorldUtil.spawnItemStack(world, x, y, z, drop);
            }
        }
    ...
    public boolean isSideSolid(IBlockAccess world, int x, int y, int z, int side) {
        ...
        if (side == facing) {
            ...
            if (tile != null && tile instanceof TileElementHolder) {
                if (((TileElementHolder) tile).hasElement()) {
                    return ((TileElementHolder) tile).getPElement()
                        .isSideSolid(world, x, y, z, side);
                }
            }
            return false;
        }
    ...
    public void getConnectedForSystem(PistonSystem system, BlockProxy proxy, boolean strongConnection) {
        ...
        if (pTile != null && pTile instanceof TileElementHolder) {
            ...
            if (element != null && element instanceof BaseBlock
                && ((BaseBlock) element).data instanceof IPistonElement) {
