...
public class PistonSystem {
                    ...
                    if (!(element.getPBlock() instanceof BRodFolder) || facing == (dir ^ 1)) {
                        ...
                        if (mechs.contains(back)) {
                            ...
                            if (back_tile != null && back_tile instanceof TileMech) {
                                ...
                                tile.setContent(
                                    mech_tile.getElement(),
                                    mech_tile.getElementMeta(),
                                    mech_tile.getElementTile());
                            }
                        } else {
    ...
    public boolean canMove() {
        ...
        for (BlockProxy element : elements) {
            ...
            if (dir == 1 && element.getCoords().y == root.getWorld()
                .getHeight() - 2) return false;
            ...
            if (staticElements.contains(element)) {
                ...
                if (block != null) {
                    if (block instanceof BRodFolder) {
                        ...
                        if (facing == dir && tile.getPInventory()
    ...
    private void sort() {
        Collections.sort(elements, new Comparator() {
            ...
            public int compare(Object arg0, Object arg1) {
                ...
                return Vector3.scalar(offset, ((BlockProxy) arg1).getCoords())
                    - Vector3.scalar(offset, ((BlockProxy) arg0).getCoords());
            }
        });
    }
                ...
                if (!isOnLine(block.getCoords(), surfaceBlocks)) {
                    ...
                    if (nextBlock.getBlock() != null && nextBlock.getMobilityFlag() == 1) {
                        ...
                        nextBlock.getBlock()
                            .dropBlockAsItem(
                                world,
                                nextBlock.getCoords().x,
                                nextBlock.getCoords().y,
                                nextBlock.getCoords().z,
                                oldMetadata,
                                0);
                    }
                }
    ...
    private boolean canElementRotate(BlockProxy element, int rotateDir) {
        ...
        Vector3 startV3 = element.getCoords()
            .copy();
                ...
                if (proxy.getBlock() != null && proxy.getMobilityFlag() == 1) {
                    ...
                    proxy.getBlock()
                        .dropBlockAsItem(
                            ...
                            0);
                    WorldUtil
                        .setBlockToAir(proxy.getWorld(), proxy.getCoords().x, proxy.getCoords().y, proxy.getCoords().z);
                }
            }
        }
    }
    ...
    private void moveAttached(BlockProxy element, int side) {
        ...
        } else if (side == dir && !element.getWorld().isRemote
            && neighbor.getBlock() != null
            && neighbor.getBlock()
                .getMobilityFlag() == 1) {
                    neighbor.dropBlockAsItem();
                }
