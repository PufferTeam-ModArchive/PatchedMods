...
public class EntityPetrifyArrow extends EntityArrowOverwrite {
    ...
    protected void onEntityLivingHit(EntityLivingBase target, NBTTagCompound nbt) {
        ...
        if (target.getHealth() <= 0) {
            ...
            if (oclass != null) {
                try {
                    statueEntity = (EntityLivingBase) oclass.getConstructor(new Class[] { World.class })
            ...
            // EntityLivingBase statueEntity = (EntityLivingBase) EntityList.createEntityFromNBT(nbt, worldObj);//
            // (EntityLivingBase) EntityList.createEntityByID(EntityList.getEntityID(target), worldObj);
            if (statueEntity != null) {
                ...
                if (!world.isRemote && ConfigData.statueSmoke)
                    PacketHandler.sendToAllInDimension(new SmokePacket(x, y, z, target.dimension), target.dimension);
