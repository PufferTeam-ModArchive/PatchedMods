...
public abstract class TileElementHolder extends PTile {
    ...
    public float getOffsetX(float ticktime) {
        return 0.0F;
        // return -1 * (this.retrieving ? (this.getProgress(prog) - 1.0F) *
        // (float)Facing.offsetsXForSide[this.getDirection()] : (1.0F - this.getProgress(prog)) *
        // (float)Facing.offsetsXForSide[this.getDirection()]);
    }

    public float getOffsetY(float ticktime) {
        return 0.0F;
        // return -1 * (this.retrieving ? (this.getProgress(prog) - 1.0F) *
        // (float)Facing.offsetsYForSide[this.getDirection()] : (1.0F - this.getProgress(prog)) *
        // (float)Facing.offsetsYForSide[this.getDirection()]);
    }

    public float getOffsetZ(float ticktime) {
        return 0.0F;
        // return -1 * (this.retrieving ? (this.getProgress(prog) - 1.0F) *
        // (float)Facing.offsetsZForSide[this.getDirection()] : (1.0F - this.getProgress(prog)) *
        // (float)Facing.offsetsZForSide[this.getDirection()]);
    }
