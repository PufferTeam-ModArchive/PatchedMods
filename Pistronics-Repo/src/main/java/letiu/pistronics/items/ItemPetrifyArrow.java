...
public class ItemPetrifyArrow extends PItem implements IArrowEventListener {
        ...
        if (entity instanceof EntityLivingBase) {
            ...
            if (target.getHealth() <= 0) {
                ...
                if (statueEntity != null) {
                    ...
                    if (!world.isRemote && ConfigData.statueSmoke) PacketHandler
                        .sendToAllInDimension(new SmokePacket(x, y, z, player.dimension), player.dimension);
    ...
    public void handleLooseEvent(ArrowLooseEvent event) {
        ...
        if (InventoryUtil.hasItemInHotbar(player, (Item) this.item)) {
            ...
            // player.worldObj.playSoundAtEntity(player, "random.bow", 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 1.2F)
            // + f * 0.5F);
