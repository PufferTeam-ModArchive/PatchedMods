...
public class ItemChisel extends PItem {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean value) {
        ...
        list.add(
            EnumChatFormatting.BLUE + "Use this to edit the details on "
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (tile != null && CompareUtil.compareIDs(stack, ItemData.chisel.item)
            && (tile.camouID != -1 || !tile.camou)) {
            if (player.isSneaking()) tile.decrementResolution();
            else tile.incrementResolution();
            if (!world.isRemote)
                PacketHandler.sendToAllInDimension(new StatueDataPacket(tile, player.dimension), player.dimension);
            return true;
        }
