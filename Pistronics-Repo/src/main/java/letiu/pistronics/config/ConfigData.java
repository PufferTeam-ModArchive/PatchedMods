...
public class ConfigData {
    ...
    public static void init() {
        // INTEGERS //
        configDataMap
            .put("maxBlocksMoving", new ConfigObject(maxSystemSize, "Maximum amount that can be moved at once."));
        configDataMap.put(
            "statueScaleMin",
            new ConfigObject(statueScaleMin, "Minimum size for statues in percent. Cannot be lower than 1."));
        configDataMap.put("statueScaleMax", new ConfigObject(statueScaleMax, "Maximum size for statues in percent."));
        configDataMap.put(
            "statueScaleStep",
            new ConfigObject(statueScaleStep, "Step by which statue scale is increased/decreased."));

        // BOOLEANS //
        configDataMap
            .put("machineSmoke", new ConfigObject(machineSmoke, "Whether machines emit smoke particles on failure."));
        configDataMap.put(
            "moveAttached",
            new ConfigObject(
                moveAttached,
        ...
        configDataMap.put(
            "creativeMachineRecipe",
            new ConfigObject(creativeMachineRecipe, "Enables recipe for creative machine."));
        ...
        configDataMap.put(
            "predictPlacement",
            new ConfigObject(predictPlacement, "Tries to predict the way you want to place rods and extensions."));

        configDataMap.put(
            "renderStatuesInInventory",
            new ConfigObject(
                renderStatuesInInventory,
                "Whether or not statues will be rendered 3D inside the inventory."));
        configDataMap.put(
            "renderStatuesAsBoxes",
            new ConfigObject(
                renderStatuesAsBoxes,
                "Prevents statues from rendering normally. Statues will be displayed as boxes. Use this if you can't enter your world after petrifying an entitiy."));

        configDataMap
            .put("statueSmoke", new ConfigObject(statueSmoke, "Whether statues emit smoke particles upon creation."));

        configDataMap.put(
            "opaqueSlimeblocks",
            new ConfigObject(opaqueSlimeblocks, "Enables this to render Slimeblocks as solid nontransparent blocks."));

        configDataMap.put(
            "showAllExtensions",
            new ConfigObject(showAllExtensions, "Whether to show all available Extensions in Creative Menu"));
        configDataMap.put(
            "showAllExtensionParts",
            new ConfigObject(showAllExtensionParts, "Whether to show all available Extensions Parts in Creative Menu"));
        configDataMap.put(
            "showAllSails",
            new ConfigObject(showAllSails, "Whether to show all available Sails in Creative Menu"));

        configDataMap.put(
            "alternateSlimeblockRecipe",
            new ConfigObject(
                alternateSlimeblockRecipe,
                "Enable to use an alternate Slimeblock recipe. (Current one conflicts with Tinkers Construct)"));
        ...
        configDataMap.put(
            ...
            new ConfigObject(
                blackListString,
                "Black List. All blocks listed here cannot be moved by pistons or rotators. "
                    + "The format is \"<unloacalized name>\" or \"<unloacalized name>:<metadata>\" devided by a ','. Don't use the prefix \"tile.\"."));
        configDataMap.put(
            "whiteList",
            new ConfigObject(
                whiteListString,
                "White List. All blocks listed here can be moved by pistons or rotators. Leave this empty if you don't want to use a whitelist. "
                    + "The format is \"<unloacalized name>\" or \"<unloacalized name>:<metadata>\" devided by a ','. Don't use the prefix \"tile.\"."));
        configDataMap.put(
            "modBlackList",
            new ConfigObject(
                modBlackListString,
                "Mod Black List. Blocks added by mods listed here cannot be moved by pistons or rotators. "
                    + "Make sure you are using modIDs not the names."));
        configDataMap.put(
            "modWhiteList",
            new ConfigObject(
                modWhiteListString,
                "Mod White List. Blocks added by mods listed here can be moved by pistons or rotators. Leave this empty if you don't want to use a whitelist. For vanilla blocks add \"minecraft\""
                    + "Make sure you are using modIDs not the names."));

        configDataMap.put(
            "fallingBlockList",
            new ConfigObject(
                fallingBlockListString,
                "Blocks listed here are treated as falling blocks (e.g. sand, gravel). The format is \"<unloacalized name>\" or \"<unloacalized name>:<metadata>\" devided by a ','. Don't use the prefix \"tile.\"."));
    }
    ...
    public static void load() {
        ...
        for (String name : names) {
            name = name.trim();
            if (!name.equals("")) fallingBlockList.add(name);
        }
