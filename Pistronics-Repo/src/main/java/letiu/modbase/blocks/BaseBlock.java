...
public class BaseBlock extends Block {
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
        ...
        if (iconKey.startsWith(BLOCK_PREFIX)) {
            ...
            return BlockItemUtil.getBlockByID(blockID)
                .getIcon(side, blockMeta);
        }
    ...
    public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z) {
        return data.shouldUseDefaultPick() ? super.getPickBlock(target, world, x, y, z)
            : data.getPickBlock(target, world, x, y, z);
    }
