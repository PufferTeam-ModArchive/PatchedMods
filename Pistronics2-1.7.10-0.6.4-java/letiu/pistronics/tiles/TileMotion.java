...
public class TileMotion extends TileElementHolder implements ISpecialRenderTile, IMover {
    ...
    public void update() {
        if (this.ctrData.controller == null) this.ctrData.controller = new SystemController(new MoveData());

        this.ctrData.updateController();
        MoveData moveData = this.ctrData.controller.getMoveData();

        if (moveData.isMoving()) pushEntities();

        if (moveData.isDone()) if (moveData.isMoving()) {
            pushEntitiesFinal();
            loadBlock();
            postMove();
    ...
    public AxisAlignedBB getBoxForPush(float pushSpace) {
        ...
        if (getPElement() instanceof letiu.pistronics.blocks.BPartblock) {
            ...
            if (box == null) box = AxisAlignedBB.getBoundingBox(x, y, z, (x + 1), (y + 1), (z + 1));
        }
        ...
        return box.getOffsetBoundingBox(
            (antiProgress * Facing.offsetsXForSide[moveData.moveDir]),
            (antiProgress * Facing.offsetsYForSide[moveData.moveDir]),
            (antiProgress * Facing.offsetsZForSide[moveData.moveDir]));
    }
    ...
    private void pushEntities() {
        ...
        PBlock block = WorldUtil.getPBlock(
            (IBlockAccess) world,
            x + Facing.offsetsXForSide[moveData.moveDir],
            y + Facing.offsetsYForSide[moveData.moveDir],
            z + Facing.offsetsZForSide[moveData.moveDir]);
        if (block != null && block instanceof letiu.pistronics.blocks.BMotionblock) {
            List list = this.tileEntity.getWorldObj()
            ...
            List list = this.tileEntity.getWorldObj()
                .getEntitiesWithinAABBExcludingEntity((Entity) null, getBoxForPush(0.25F));
            Iterator<Entity> itr = list.iterator();
            while (itr.hasNext()) {
                Entity entity = itr.next();
                entity.moveEntity(
                    (moveAmt * Facing.offsetsXForSide[moveData.moveDir]),
                    (moveAmt * Facing.offsetsYForSide[moveData.moveDir]),
                    (moveAmt * Facing.offsetsZForSide[moveData.moveDir]));
                switch (moveData.moveDir) {
                    case 0:
                    ...
                    case 1:
                        if (entity.motionY < 0.0D) entity.motionY = 0.0D;
                    case 2:
                        if (entity.motionZ > 0.0D) entity.motionZ = 0.0D;
                    case 3:
                        if (entity.motionZ < 0.0D) entity.motionZ = 0.0D;
                    case 4:
                        if (entity.motionX > 0.0D) entity.motionX = 0.0D;
                    case 5:
                        if (entity.motionX < 0.0D) entity.motionX = 0.0D;
                }
            }
    ...
    private void pushEntitiesFinal() {
        MoveData moveData = this.ctrData.controller.getMoveData();
        List list = this.tileEntity.getWorldObj()
            .getEntitiesWithinAABBExcludingEntity((Entity) null, getBoxForPush(0.0F));
        Iterator<Entity> itr = list.iterator();
        while (itr.hasNext()) {
            Entity entity = itr.next();
            switch (moveData.moveDir) {
    ...
    public float getAngleForRender(float ticktime) {
        MoveData moveData = this.ctrData.controller.getMoveData();
        if (ticktime > 1.0F) ticktime = 1.0F;
    ...
    private void postRotate() {
        ...
        if (block instanceof ISpecialRotator) {
            ((ISpecialRotator) block)
                .postRotate(world, x, y, z, moveData.rotateDir, moveData.rotateSpeed, moveData.rotatePoint);
        } else {
    ...
    public void writeToNBT(NBTTagCompound tagCompound) {
        ...
        if (this.ctrData.key == null && this.ctrData.controller != null)
            this.ctrData.key = this.ctrData.controller.getKey();
        if (this.ctrData.key == null) this.ctrData.key = "default";
